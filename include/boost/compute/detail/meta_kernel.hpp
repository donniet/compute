...
public:
    ...
    // define stream operators for long, ulong and uint scalar and vector types
    meta_kernel& operator<<(const long_ &x)
    {
        m_source << std::showpoint << x << 'l';
        return *this;
    }
    meta_kernel& operator<<(const ulong_ &x)
    {
        m_source << std::showpoint << x << "ul";
        return *this;
    }
    meta_kernel& operator<<(const uint_ &x)
    {
        m_source << std::showpoint << x << 'u';
        return *this;
    }
    ...
    BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(uint2_)
    BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(uint4_)
    BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(uint8_)
    BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(uint16_)
